...
describe('AkuManga Tests', function () {
    ...
    var source: Source = new AkuManga(cheerio);
    ...
    it("Testing Search", async () => {
        let searchQuery: SearchRequest = createSearchRequest({title: "God"})
        let searchResults = await wrapper.search(source, searchQuery, 1)

        expect(searchResults, "No server response").to.exist
    })
